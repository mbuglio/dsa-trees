...
class BinaryTree {
  ...
  maxDepth() {
    ...
    function maxDepthHelpter(node) {
      if(node.left === null && node.right === null) return 1;
      if(node.left === null) return maxDepthHelpter(node.right) + 1;
      if(node.right === null) return maxDepthHelpter(node.left) + 1;
      return(
        Math.max(maxDepthHelpter(node.left), maxDepthHelpter(node.right)) + 1
      );
    }
    return maxDepthHelpter(this.root);
  }
